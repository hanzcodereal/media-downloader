...
import { Download, Link as LinkIcon, AlertCircle, CheckCircle2, X, Loader2, Sparkles, Server, ShieldCheck, Globe, FileDown, Check } from "lucide-react";
...
export default function Home() {
  ...
  const handleDirectDownload = async (mediaUrl: string, label: string) => {
    if (downloadingFiles.has(mediaUrl)) return;
    
    setDownloadingFiles(prev => new Set(prev).add(mediaUrl));
    setError(null);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          throw new Error(json.error || `Download failed with status ${response.status}`);
        } catch {
      ...
      // Check if blob is HTML error page
      if (blob.type.includes('text/html')) {
        const text = await blob.text();
        if (text.includes('<!DOCTYPE') || text.includes('<html')) {
          throw new Error('Server returned HTML instead of file. URL may be invalid.');
        }
      }
      ...
      let filename = `mediadownn_${Math.floor(Math.random() * 9000000000 + 1000000000)}.mp4`;
      ...
      setDownloadedFiles(prev => new Set(prev).add(mediaUrl));
    ...
    <div className="flex flex-col min-h-screen w-full bg-[#E2E8F0]">
      <header className="h-[60px] sticky top-0 z-40 bg-[#E2E8F0]/90 backdrop-blur-md border-b flex items-center px-4 md:px-[24px] border-[#D0D5D2]">
        <div className="w-full max-w-[1440px] mx-auto flex items-center justify-between">
          ...
          <nav className="hidden sm:flex items-center gap-[24px]">
            ...
            <a href="https://saweria.co/hanzreally" target="_blank" rel="noopener noreferrer" className="text-[#020817] text-[14px] font-medium leading-[22px] py-[8px] px-[12px] border-b-2 border-transparent hover:text-[#1863DC] hover:border-[#FF9E33] transition-colors">
              Support
            </a>
          </nav>
        </div>
      ...
      <main className="flex-1 w-full max-w-[1440px] mx-auto px-4 sm:px-[24px] md:px-[40px] py-[40px] md:py-[80px] flex flex-col gap-[24px] md:gap-[32px]">
        ...
        <section className="flex flex-col items-center mb-[16px] md:mb-[32px] text-center">
          ...
          >
            Retrieve videos and media from your favorite social platforms instantly.
          </motion.p>
              ...
              <div className="w-full max-w-3xl bg-[#FFFFFF] p-[24px] md:p-[32px] rounded-[9px] border border-[#E2E8F0] shadow-[0px_4px_12px_rgba(0,0,0,0.1)]">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-[24px] border-b border-[#EBEBEB] pb-[16px] gap-[12px]">
                  <div>
                    ...
                    <p className="text-[#212121] text-[14px] mt-[4px]">
                      {links.length} {links.length === 1 ? 'file' : 'files'} available for download
                    </p>
                  </div>
                ...
                {links.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-[16px] mb-[24px]">
                    {links.map((link, idx) => {
                      const isDownloading = downloadingFiles.has(link.url);
                      const isDownloaded = downloadedFiles.has(link.url);
                      ...
                      return (
                        <motion.div 
                          ...
                          key={idx} 
                          className={`flex flex-col justify-between p-[20px] border rounded-[6px] bg-[#F4F4F4] transition-all group ${
                            isDownloaded 
                              ? 'border-[#008000] shadow-[0px_4px_12px_rgba(0,128,0,0.2)]' 
                              : 'border-[#D0D5D2] hover:border-[#1863DC] hover:shadow-[0px_4px_12px_rgba(0,0,0,0.05)]'
                          }`}
                        >
                          <div className="flex items-start gap-[12px] mb-[16px]">
                            <div className={`w-[40px] h-[40px] bg-[#FFFFFF] rounded-[4px] border flex items-center justify-center shrink-0 shadow-sm ${
                              isDownloaded ? 'border-[#008000]' : 'border-[#EBEBEB]'
                            }`}>
                              {isDownloaded ? (
                                <Check className="w-[20px] h-[20px] text-[#008000]" />
                              ) : isDownloading ? (
                                <Loader2 className="w-[20px] h-[20px] text-[#1863DC] animate-spin" />
                              ) : (
                            ...
                            <div className="flex-1 overflow-hidden">
                              ...
                              <p className={`text-[12px] truncate mt-[2px] ${
                                isDownloaded ? 'text-[#008000] font-medium' : 
                                isDownloading ? 'text-[#1863DC] font-medium' : 
                                'text-[#212121] opacity-70'
                              }`}>
                                {isDownloaded ? '✓ Downloaded successfully' : 
                          ...
                          <button
                            onClick={() => handleDirectDownload(link.url, link.label)}
                            disabled={isDownloading || isDownloaded}
                            className={`w-full h-[40px] flex items-center justify-center gap-[8px] rounded-[2px] text-[14px] font-medium transition-all ${
                              isDownloaded
                                ? 'bg-[#008000] text-white border-2 border-[#008000] cursor-default'
                                : isDownloading
                                ? 'bg-[#FF9E33] text-white border-2 border-[#FF9E33] cursor-wait opacity-70'
                                : 'bg-[#1863DC] text-white border-2 border-[#1863DC] hover:bg-[#1a6be0] hover:border-[#1a6be0] active:bg-[#1554b5] active:border-[#1554b5]'
                            }`}
                          >
                            {isDownloaded ? (
                              <>
                                <Check className="w-[16px] h-[16px]" /> Downloaded
                              </>
                            ) : isDownloading ? (
                              <>
                                <Loader2 className="w-[16px] h-[16px] animate-spin" /> Downloading...
                              </>
                            ) : (
                              <>
                                <FileDown className="w-[16px] h-[16px]" /> Download Now
                              </>
                            )}
                          </button>
                        </motion.div>
        ...
        <section className="w-full max-w-4xl mx-auto mt-[16px] md:mt-[32px] grid grid-cols-1 md:grid-cols-3 gap-[24px]">
          <div className="md:col-span-1 flex flex-col gap-[24px]">
            <div className="bg-[#FFDBC4] p-[24px] rounded-[9px] text-[#38102A] h-full shadow-[0px_4px_12px_rgba(0,0,0,0.1)] flex flex-col items-start">
              ...
              <p className="text-[14px] font-normal leading-[21px] flex-1">
                Servers optimized globally to fetch your content with ultra-low latency.
              </p>
            </div>
          </div>
          ...
          <div className="md:col-span-2 bg-[#FFFFFF] p-[24px] md:p-[32px] rounded-[9px] border border-[#E2E8F0] shadow-[0px_4px_12px_rgba(0,0,0,0.1)]">
            ...
            <p className="text-[14px] font-normal leading-[24px] text-[#212121]">
               Supports: TikTok, Douyin, Capcut, Threads, Instagram, Facebook, YouTube, Twitter, Reddit, Spotify, and many more.
            </p>
          </div>
        </section>
        ...
        <div className="max-w-[1440px] mx-auto flex flex-col md:flex-row items-center justify-between gap-[24px]">
          <div className="flex flex-col items-center md:items-start gap-[8px]">
            ...
            <p className="text-[14px] font-normal leading-[24px] text-[#212121] max-w-sm text-center md:text-left">
              Enterprise-grade media extraction tooling.
            </p>
          </div>
          
          <div className="flex flex-col items-center md:items-end md:text-right gap-[4px]">
            <p className="text-[14px] font-normal leading-[24px] text-[#212121]">
              Developed & Maintained by <span className="font-medium text-[#020817]">HANZZ CODEE</span>
            </p>
            <p className="text-[12px] font-medium leading-[20px] text-[#1863DC] uppercase tracking-wider mb-[4px]">
              HANZZ CODEE © {new Date().getFullYear()}
            </p>
            <a href="https://saweria.co/hanzreally" target="_blank" rel="noopener noreferrer" className="text-[#1863DC] text-[14px] font-medium leading-[24px] hover:underline hover:underline-offset-4 flex items-center gap-[6px]">
              Support Us
            </a>
          </div>
        </div>
        ...
        {showPopup && (
          ...
          >
            ...
            >
              ...
              <h3 className="text-[32px] font-medium leading-[1.2] mb-[12px] tracking-tight">Support Us</h3>
              <p className="text-[16px] font-normal leading-[25px] mb-[32px] text-[#FFFFFF]/80">
                Support HANZZ CODEE development through Saweria.
              </p>
